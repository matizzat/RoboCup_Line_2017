...
//  coso.Enable(1500,20000);
  ...
  while(true){
    ...
    ((RAZON_RGB_I || RAZON_RGB_D) && tiempo > 500)) && (x_inicial - (fabs(angle[0])) < 2.4f))
    {
      ...
      motores(vel_adelante+(0.15*a_la_rampa)-(0.3*bajando)-(0.15*en_rampa),vel_adelante+(0.15*a_la_rampa)-(0.3*bajando)-(0.15*en_rampa));
    }
  ...
  /*  if((fabs(angle[0]) < 67.0f && angle[0] < 0) && a_la_rampa == false){
    ...
    if((en_rampa == true && fabs(angle[0]) < 72.0f && angle[0] > 0) && bajando == false){
      bajando = true;
    }*/
  }
}
