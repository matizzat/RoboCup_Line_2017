...
#define N_INT_IZQ 600
#define N_INT_DER 380//980
...
///////////////////////////////////////////////
#define I_CALR 0.4733f//0.3892f
#define I_CALG 0.5691f//0.465f
#define I_CALB 0.4308f//0.365f
...
/*void obstaculo(){
  ...
  if(esquivar == 1){
    motores(-0.3,0.3);
    while(distancia_der >8){
      distancia_der = u_der.distance();
      wait_ms(40.0f);
    }
    distancia_der = u_der.distance();
    while(distancia_der < 9 && distancia_der != 0){
      distancia_der = u_der.distance();
      wait_ms(60.0f);
...
int main(){
  ...
  pala.enable(-50);                        //LLEVAR LA PALA HACIA ARRIBA
  tu.attach_us(&updateMotors,100.0f);   //ATTACH DE TICKERS
  angle[0] = -87.0f;
  muestreo_mpu();
  pc.baud(115200);
  wait(3.0f);
  ...
  while(true){
    ...
    if(estado == seguidor){
      if((fabs(angle[0]) < 70.0f && angle[0] < 0) && (fabs(angle_ant) < 66.0f && angle_ant < 0)){
      ...
      else if(en_rampa == true && (fabs(angle[0]) < 83 && angle[0] > 0)){
        motores(0,0);
        wait(2.0f);
        ...
        ((RAZON_RGB_I || RAZON_RGB_D) && tiempo > 500)) && fabs(angle[0]) > 85.0f)
      {
      ...
      if(fabs(angle[0]) > 78.0f && angle[0] > 0){
        motores(0.0,0.0);
        a_la_rampa = false;
        en_rampa = true;
        wait(1.0f);
      ...
      if(s_izq < N_IZQ && s_der > N_DER) {
        ...
        else if(fabs(angle[0]) < 64){
          motores(0.3,0.3);
        }
        else if(fabs(angle[0]) < 70){
          motores(0.33,0.33);
        }
        else if(fabs(angle[0]) < 74){
          motores(0.35,0.35);
        }
        else if(fabs(angle[0]) < 77){
          motores(0.37,0.37);
        }
        else if(fabs(angle[0]) < 80){
          motores(0.4,0.4);
        }
      }
    }
